...
function SearchComponent() {
  ...
  async function handleChange(e) {
    ...
    let result = await searchMovies(e.target.value).get("/search/movie");
    console.log("Searched");
    console.log(result.data.results);
    setDisplaySearchField((prevState)=>prevState=true);
  ...
  function closeSearchField(){
    setDisplaySearchField((prevStateValue)=> prevStateValue = false);
  }
  function openSearchField(){
    setDisplaySearchField((prevStateValue)=>prevStateValue = true);
  }
  ...
  return (
    ...
    <div className={styles.searchbox}>
      <img
        src="https://img.icons8.com/material-outlined/24/808080/search--v1.png"
        style={{ height: "1rem", width: "", color: "gray", marginLeft: "2px" }}
      />
      <input
       ref ={inputRef}
        type="text"
    ...
    <div className={styles.searchdropdown}>
     { displaySearchField ? searchResults.map((data)=>(<div className = {styles.searchtile} key= {data.id} onClick={(event)=>handleClick(event,data.title)}>{data.title}</div>)) : null}
   </div>
    </div>
...
export function BookMyShowSvg() {
  return (
    <svg
      className={`${styles.bookmyshowsvg} `}
      viewBox="0 0 88 26"
      height="33.9"
      width="115"
...
export function TitleComponent() {
  return (
    <h2 className={`${styles.titlecomponent} ${styles.mobileview}`}>
      It All Starts Here!
    </h2>
  );
}
...
export function LocationButton() {
  return <p className={styles.locationbutton}>Chennai</p>;
}
export function DropDownButton() {
  return <p className={styles.dropdownbutton}>Chennai</p>;
}
...
export function UseAppButton() {
  return (
    <button className={`${styles.signinbutton} ${styles.mobileview}`}>
      Use App
    </button>
  );
}
